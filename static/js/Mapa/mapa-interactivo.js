...
class MapaInteractivo {
  ...
  crearMapa() {
    this.mapa = L.map("mapa", {
      center: [
        this.coordenadasDefecto.latitud,
        this.coordenadasDefecto.longitud,
      ],
  ...
  /**
   * Carga en paralelo puntos ECA y ArcGIS, unifica con origen
   */
  cargarPuntosECAyArcGIS() {
    console.log("📍 Cargando puntos ECA y ArcGIS...");
    this.mostrarIndicadorCarga(true);
    const fetchEca = fetch("/mapa/api/puntos-eca")
      .then((r) => r.json())
      .then((data) => data.map((pto) => ({ ...pto, source: "eca" })));
    const fetchArcgis = fetch("/mapa/api/arcgis/puntos/")
      .then((r) => r.json())
      .then((data) =>
        data.map((pto) => ({
          puntoEcaID: pto.id
            ? "arcgis_" + pto.id
            : "arcgis_" + (pto.latitud + "," + pto.longitud),
          latitud: pto.latitud,
          longitud: pto.longitud,
          nombrePunto: pto.nombre || "",
          localidadNombre: pto.localidad || "",
          direccion: pto.direccion || "",
          celular: "",
          email: pto.email || "",
          horarioAtencion: "",
          source: "arcgis",
          estado: pto.estado || "",
          representante_legal: pto.representante_legal || "",
          contacto: pto.contacto || "",
        })),
      );
    Promise.all([fetchEca, fetchArcgis])
      .then(([puntosECA, puntosArcGIS]) => {
        this.puntosECA = [...puntosECA, ...puntosArcGIS];
        this.renderizarMarcadores();
        this.renderizarLista();
        this.actualizarContadores();
        console.log(
          `✅ Total puntos en mapa: ${this.puntosECA.length} (ECA: ${puntosECA.length}, ArcGIS: ${puntosArcGIS.length})`,
        );
      })
      .catch((error) => {
        console.error("❌ Error al cargar puntos:", error);
        this.mostrarError(
          "Error al cargar los puntos. Por favor, intenta más tarde. " +
            error.message,
        );
      })
      .finally(() => {
  ...
  cargarPuntosECA() {
    ...
    fetch("/mapa/api/puntos-eca")
      ...
      .catch((error) => {
        ...
        this.mostrarError(
          "Error al cargar los puntos ECA. Por favor, intenta más tarde. Error: " +
            error.message,
        );
      })
      .finally(() => {
   ...
   */
  crearMarcador(punto) {
    // Crear icono personalizado según origen del punte
  ...
  seleccionarPunto(puntoId) {
    ...
    const tarjeta = document.querySelector(
      `.tarjeta-punto[data-punto-id="${puntoId}"]`,
    );
    if (tarjeta) {
  ...
  mostrarModalDetalles(detalles) {
    ...
    document.getElementById("detalleLocalidad").textContent =
      detalles.localidadNombre || "No especificada";
    document.getElementById("detalleDireccion").textContent =
      detalles.direccion || "No especificada";
  ...
  buscar(termino) {
    ...
    document.getElementById("contadorActual").textContent =
    ...
    fetch("/mapa/api/materiales")
      ...
      .then((materiales) => {
        ...
        selectMaterial.innerHTML =
          '<option value="">Seleccionar material...</option>' + opcionesHTML;
  ...
  aplicarFiltros() {
    ...
    const inputNombre = document
      .getElementById("inputBusquedaNombre")
      .value.toLowerCase();
  ...
  filtrarPorMaterial(materialId, terminoNombre) {
    ...
    fetch(`/mapa/api/puntos-eca/por-material/${materialId}`)
      ...
      .then((puntosConMaterial) => {
        console.log(
          `✅ Se encontraron ${puntosConMaterial.length} puntos con el material`,
        );
