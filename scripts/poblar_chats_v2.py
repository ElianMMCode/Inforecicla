...
def random_biz_day(biz_days):
    return random.choice(biz_days)  # NOSONAR

def random_time_on(day):
    h = random.randint(8, 17)  # NOSONAR
    m = random.randint(0, 59)  # NOSONAR
...
def create_mensajes(chat, ciudadano, gestor, biz_days):
    n = random.randint(3, 10)  # NOSONAR
    day = random_biz_day(biz_days)
    t0 = random_time_on(day)

    for i in range(n):
        offset = datetime.timedelta(minutes=random.randint(5, 180) * i)  # NOSONAR
        dt = t0 + offset
        if dt.date() > datetime.date(2026, 6, 22):
            dt = dt - datetime.timedelta(days=random.randint(1, 7))  # NOSONAR

        if i % 2 == 0:
            rem = ciudadano
            txt = random.choice(TEXTS_CIU)  # NOSONAR
        else:
            rem = gestor
            precio = random.randint(500, 6000)  # NOSONAR
            txt = random.choice(TEXTS_ECA).replace('{precio}', str(precio))  # NOSONAR
        ...
        if i == n - 1 and random.random() < 0.3:  # NOSONAR
