...
def create_chat_and_messages(punto, ciudadano, gestor_eca):
    ...
    n_mensajes = random.randint(3, 10)  # NOSONAR
    fecha_base = datetime.datetime(2026, 5, random.randint(1, 31), random.randint(8, 17), random.randint(0, 59))  # NOSONAR

    for i in range(n_mensajes):
        if i % 2 == 0:
            remitente = ciudadano
            texto = random.choice(MENSAJES_CIUDADANO)  # NOSONAR
        else:
            remitente = gestor_eca
            precio = random.randint(200, 5000)  # NOSONAR
            direccion = punto.direccion or "nuestra sede"
            telefono = punto.celular or "nuestro teléfono"
            texto = random.choice(RESPUESTAS_ECA).replace('{precio}', str(precio)).replace('{direccion}', direccion).replace('{telefono}', telefono)  # NOSONAR

        fecha = fecha_base + datetime.timedelta(minutes=random.randint(1, 120) * i)  # NOSONAR
        if fecha.date() > datetime.date(2026, 6, 22):
            fecha = make_aware(datetime.datetime(2026, 6, 22, random.randint(8, 17), random.randint(0, 59)))  # NOSONAR

        es_leido = True
        if i == n_mensajes - 1 and random.random() < 0.3:  # NOSONAR
            es_leido = False
        ...
        Mensaje.objects.create(
            ...
            es_leido=es_leido,
            es_editado=random.random() < 0.05,  # NOSONAR
        )
